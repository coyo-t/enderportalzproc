...
mat3 end_portal_layer2 (float layer)
{
	...
	mat2 scale = mat2(((4.5 - (layer*0.75)) / 4.0) * 2.0);
...
void main ()
{
	...
	{
		vec2 yeow2 = (yeowch)*(v_incoming.y-((1.0-float(i)/15.0)+cos(GameTime*0.5)*0.1)*2.0-(0.25+(sin(GameTime)*0.5+0.5)*0.2))-v_incoming.xz;
		vec3 cc = samplec(i) * ((1.0 - float(i) / 15.0) * 0.5 + 0.5);
		vec3 sam = end_portal_layer2(float(i+1)) * vec3(yeow2, 1.0);
		vec3 cc2 = texture2D(sampler1, (sam.xy+vec2(0.0, GameTime*0.05+float(i))) * 0.15).rgb * cc;
		
		float lenn = max(length(yeow2)-0.5, 0.0)*0.45;
		color += mix(vec3(0.0), cc2, clamp(1.0-pow(lenn, 4.2), 0.0, 1.0));
		color += clamp(dot(floor(abs(yeow2/3.0)),vec2(1.0)),0.0,1.0)*samplec(i)*0.25;
	}
